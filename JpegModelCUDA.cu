...
/*this function is only used in c++11*/
int stoi(char *c) {
	int sum = 0;
	for (int i = 0; i < 4; i++) {
		if (c[i] <= '9' && c[i] >= '0') {
...
int Qmatrix8[BLOCK][BLOCK] = { { 16, 11, 10, 16, 24, 40, 51, 61 }, { 12, 12, 14,
...
//	cout << endl;
	...
	/*write image file*/
	pgmWrite((char *) "yongJPG30.pgm", matrixTmp, szx, szy);

	/*destroy*/
	cudaFree(d_Encodearr);
	cudaFree(d_Qmatrix8);
	cudaFree(d_matrix);
	cudaFree(d_outarr);
	cudaDeviceReset();
