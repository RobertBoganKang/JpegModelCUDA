...
__device__ void FourierDCT(int *matrix, int *omatrix, int *Qmatrix8, int idx,
                           int idy, int row, int col, int IMGX) {
        ...
        for (int u = 0; u < BLOCK; u++) {
            ...
            float alpha = (float) (
                    ...
                    (u == 0 && v == 0 ?
                     ...
                     sqrtf((float) (1.0 / 2.0))) :
                    1.0);
            tempmtr2[u * BLOCK + v] = (float) (1.0 / 4.0 * alpha
...
__device__ void InverseFourierDCT(int *matrix, int *Qmatrix8, int idx, int idy, int row, int col, int IMGX) {
    ...
    matrix[idx * IMGX + idy] = (int) round(
            1.0 / 4.0 * tempmtr2[row * BLOCK + col] + 127);
    __syncthreads();
}

/*GPU operations*/
__global__ void CUDA_MAIN(int *matrix, int *oarr, int *Qmatrix8, int *idxarr, int len, int blockX, int IMGX) {
    ...
    /**encoder*/
    FourierDCT(matrix, omatrix, Qmatrix8, idx, idy, row, col, IMGX);
...
int main() {
    int szx = 0, szy = 0;
    int **matrixTmp = NULL;
    matrixTmp = pgmOpen((char *) "yong.pgm", szx, szy);
    int matrix[szx][szy];
    for (int i = 0; i < szy; ++i) {
        for (int j = 0; j < szx; ++j) {
...
//	cout << endl;
    ...
    /*write image file*/
    pgmWrite((char *) "yongJPG.pgm", matrixTmp, szx, szy);
